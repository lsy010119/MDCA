...
class MDCA:
    ...
    def split_segment(self,interval):
        ...
        for i in range(K):
            ...
            for n in range(len(self.UAVs[i].wp)-1):
                ...
                for j in range(1,ip_num+1):

                    insert_loc = (j/ip_num)*self.UAVs[i].wp[n].loc - (1 - j/ip_num)*self.UAVs[i].wp[n+1+ip_count].loc

                    insert_point = WP(insert_loc, i)

                    self.UAVs[i].wp.insert(n+j+1,insert_point)
    ...
    def run(self, avoidance=True, simul_arr=True):
        ...
        self.split_segment(1)

        for i in range(len(self.UAVs[0].wp)):

            plt.scatter(self.UAVs[0].wp[i].loc[0],self.UAVs[0].wp[i].loc[1])
